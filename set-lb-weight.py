...
import os, socket, sys, subprocess, memcache

# Let's find the current state of the load-balancer
SOCKET = '/etc/haproxy/haproxy.sock'
status = subprocess.getstatusoutput('echo "show stat" | socat %s stdio' % SOCKET )
...
if command:
    ...
    status = subprocess.getstatusoutput('echo "%s" | socat stdio %s' % ( '; '.join( command ), SOCKET ) )
